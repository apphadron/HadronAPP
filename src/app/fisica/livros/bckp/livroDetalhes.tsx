        ...
        const fetchBookDetails = async () => {
            ...
            try {
                ...
                if (error) {
                    ...
                    // Tentar buscar do cache se houver erro na requisição
                    const cachedData = await AsyncStorage.getItem(`bookDetails_${id}`);
                    if (cachedData) {
                        setBookDetails(JSON.parse(cachedData));
                    } else {
                        // Se não tiver no cache, criar um objeto mínimo
                        setBookDetails({
                            id,
                    ...
                    const completeData = {
                        ...
                        resumo: data.resumo || "Resumo não disponível.",
                        edicao: data.edicao || "Não informado",
                        data_publicacao: data.data_publicacao || "Não informado",
                        genero: data.genero || "Não informado",
                        classificacao: data.classificacao_idade || "Não informado",
                        idioma: data.idioma || "Não informado",
                        buy_link: data.buy_link || null,
                        rating: data.rating || 4.5,
                        paginas: data.paginas || "Não informado"
                    };
